...
const Experience = ({ title, place, duration, details, achievements, technologies }) => (
  <div>
    <h3><BsArrowRightCircleFill /> {title} at <a target='_blank' href='https://www.univ-constantine2.dz/facntice/' style={location}>{place}</a></h3>
    <p>for {duration}</p>
    <p>{details}</p>
    ...
    <div style={main}>
      ...
      <div style={Container}>
        {skill.map((skill, index) => (
          <div key={index} style={skillBox} id='skillbox'>
            {skill}
          </div>
        ))}
      </div>
